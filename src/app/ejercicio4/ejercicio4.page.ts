...
export class Ejercicio4Page implements OnInit {
  ...
  subirPartido(partido:Partido){
    if(!partido.disputado){
      this.presentAlert("Solo se pueden subir a Firebase partidos ya disputados");
      return;
    }
    ...
    .then((partidoDevuelto)=>{
      if(!partidoDevuelto){
        ...
        .then((response)=>{
          this.presentAlert('Inserción correcta en firebase');
        })
        .catch((error)=>{
          console.log(error);
        });
        ...
        this.presentAlert('Ya se ha subido el partido a firebase');
      }
    })
    .catch((error)=>{
      console.log(error);
